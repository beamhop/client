 ...
 * note (kind 1) and a documentation (kind 30023 + beamhop-doc marker) to the real
...
const idJson = JSON.parse(await Bun.file("/tmp/beamhop-shot/identity.json").text()) as {
  kind: "local";
  secretKey: string;
};
...
const note: EventTemplate = {
  ...
  created_at: now,
  tags: [],
  content: `Beamhop smoke test — live note published from the new client at ${stamp} 🔑`,
};
const docId = `beamhop-smoke-${now}`;
const doc: EventTemplate = {
  kind: 30023,
  created_at: now,
  tags: [
    ["d", docId],
    ["title", "Beamhop client — it works"],
    ["summary", "A documentation published end-to-end from the new Beamhop Nostr client."],
    ["published_at", String(now)],
    ["t", "beamhop-doc"],
    ["t", "beamhop"],
  ],
  content: `# It works\n\nThis NIP-23 documentation was published live from **Beamhop**.\n\n## Markdown renders\n\n- Lists\n- \`code\`\n- > and quotes\n\nVerified at ${stamp}.`,
};
